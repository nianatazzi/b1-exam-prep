...
1. **theory** — блоки сортируются по полю `id`. Каждый блок сразу сопровождается своими упражнениями (`segment_type = "theory"`, `linked_item_id = id` блока).
2. **lexical_set** — сначала подряд показываются ВСЕ блоки коллекции (сортировка по `id`), без упражнений между ними. После последнего блока — все упражнения (`segment_type = "vocab"`) для всех блоков лексики этого урока.
...
Упражнения внутри одной группы сортируются по полю `id` документа упражнения.
